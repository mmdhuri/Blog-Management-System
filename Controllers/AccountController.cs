...
namespace Student_Management_System.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(IApplicationUserDetailsService applicationUserDetailsService)
        {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            //        return View(model);
            //}
            var UserData = db.ApplicationUsersDetails.Where(m => m.EmailId == model.Email && m.Deleted == true).ToList();
            if (UserData.Count() > 0)
            {
                ModelState.AddModelError("", "Sorry User have been deleted.");
                //model.ReturnUser = ReturnUser;
                return View(model);
            }

            HttpCookie cookie = new HttpCookie("RememberMeInfo");
            //ViewBag.ReturnUser = ReturnUser;
            //model.ReturnUser = ReturnUser;
            var RememberMeCheckbox = model.RememberMe;
            if (RememberMeCheckbox == false)
            {
                cookie["RememberMeEmail"] = "";
            }
            else
            {
                cookie["RememberMeEmail"] = model.Email;
            }

            // This cookie will remain  for one month.
            //cookie.Expires = Infrastructure.TimeZone.GetDateTime().AddMonths(1);
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    /*string URL = "" + data.FirstName + "_" + data.LastName;
                    Session["ProURL"] = URL;*/
                    if (Session["ApplicantVeriyid"] != null)
                    {
                        //return Redirect("/CandidateJobList/List?id=1");
                        if (returnUrl != null)
                        {
                            return RedirectToLocal(returnUrl);
                        }
                        ...
                        {
                            return RedirectToAction("ApplicantVerificationAdd", "ApplicantVerification");
                        }
                    }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    ApplicationUsersDetail appUser = new ApplicationUsersDetail();

                    appUser.FirstName = model.FirstName;
                    appUser.LastName = model.LastName;
                    appUser.EmailId = model.Email;
                    appUser.AspNetUserId = user.Id;
                    appUser.CreatedBy = user.Id;
                    appUser.UpdatedBy = user.Id;
                    appUser.CreatedDate = DateTime.Now;
                    appUser.UpdatedDate = DateTime.Now;
                    _applicationUserDetailsService.Insert(appUser);
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        // Extract user details from external identity claims
                        ...
                        // Fallback to splitting the full name if claims are not available
                        if (string.IsNullOrEmpty(firstName) || string.IsNullOrEmpty(lastName))
                        {
                            var nameParts = info.ExternalIdentity.Name.Split(' ');
                        ...
                        _applicationUserDetailsService.Insert(appUser);
                        UserManager.AddToRole(user.Id, "User");
                        await UserManager.AddPasswordAsync(user.Id,"M$23anish@");
                        ...
                        await _sendEmails.SendEmailAsync(model.Email, "Confirm your account", body);
                        return RedirectToAction("ForgotPasswordConfirmation", "Account");
                    }
                }
                AddErrors(result);
